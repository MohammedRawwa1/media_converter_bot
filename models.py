...
class MediaConversionModel:
    ...
    async def _update_user_stats(self, conversion_data: Dict[str, Any]):
        ...
        try:
            ...
            update_data = {
                "$inc": {
                    f"stats.total_conversions": 1,
                    f"stats.{action}": 1,
                    f"stats.total_input_size": conversion_data.get('input_size', 0),
                    f"stats.total_output_size": conversion_data.get('output_size', 0)
                },
                "$set": {
                    "last_activity": datetime.utcnow(),
                    "username": conversion_data.get('username')
                },
                "$setOnInsert": {
                    "user_id": user_id,
    ...
    async def get_user_stats(self, user_id: int) -> Optional[Dict]:
        ...
        try:
            user_data = await self.users.find_one(
                {"user_id": user_id},
                {"_id": 0, "stats": 1, "first_seen": 1, "last_activity": 1}
            )
            return user_data
        except Exception as e:
    ...
    async def get_daily_stats(self, date: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            stats_data = await self.stats.find_one(
                {"date": date},
                {"_id": 0}
            )
    ...
    async def cleanup_old_data(self, days: int = 30) -> int:
        ...
        try:
            ...
            result = await self.conversions.delete_many({
