...
def setup_handlers(application: Application) -> None:
    ...
    application.add_handler(MessageHandler(filters.VIDEO, handler_manager.handle_media_message))
    application.add_handler(MessageHandler(filters.AUDIO, handler_manager.handle_media_message))
    application.add_handler(MessageHandler(filters.DOCUMENT, handler_manager.handle_media_message))
...
async def main() -> None:
    ...
    if not check_ffmpeg_available():
        logger.warning("FFmpeg binary not found or not executable. Conversions may fail on startup.")
    else:
