...
async def websocket_progress(
    websocket: WebSocket,
    job_id: str,
    job_token: str | None = Query(None),
):
    """Stream progress events for a job via WebSocket.

    Authorization is the job's own access capability (``?job_token=``), minted
    when the job was created — see utils/job_access.py. A browser cannot set
    headers on a WebSocket handshake, which is why the capability travels in the
    query string: it is per-job and unguessable, so it reads like a signed URL,
    unlike the shared upload secret, which no longer appears in any URL.
    Fails closed when the job has no recorded capability.
    """

    # Rate limiting: a handshake flood is cheap to send and not cheap to hold.
    client_ip = get_client_ip(websocket)
    if not web_rate_limiter.check_limit("ws", client_ip):
        await websocket.close(code=4029, reason="rate limited")
        return

    # Auth: reject before accepting.
...
async def sse_events(
    job_id: str,
    request: Request,
    job_token_header: str | None = Header(None, alias="X-Job-Token"),
    job_token_query: str | None = Query(None, alias=JOB_CAPABILITY_PARAM),
):
    ...
    Authorization is the job's own access capability, as for ``/ws/{job_id}``:
    the ``X-Job-Token`` header for any client that can set one, or ``?job_token=``
    for ``EventSource``, which cannot. Fails closed when the job has none.
    """

    # Rate limiting: an SSE connection holds a Redis subscription for its life.
    client_ip = get_client_ip(request)
    if not web_rate_limiter.check_limit("sse", client_ip):
        body, code, headers = make_rate_limit_response("sse", client_ip)
        return JSONResponse(status_code=code, content=body, headers=headers)

    # Auth: this job's capability, and it fails closed.
    incoming_token = job_token_header or job_token_query
    if not await job_token_ok(job_id, incoming_token):
        return JSONResponse(
            status_code=401,
            content={"error": "unauthorized", "detail": "missing or invalid job token"},
        )
