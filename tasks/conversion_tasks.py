...
try:
    from utils.async_timeout_wrapper import run_subprocess_with_timeout, DEFAULT_FFMPEG_TIMEOUT
except ImportError:
...
async def convert_video_to_mp3(input_path: str, output_path: str, bitrate: str = "192k", timeout_seconds: int = 18000) -> Tuple[bool, str]:
...
async def compress_video(input_path: str, output_path: str, preset: str = "medium", crf: int = 23, timeout_seconds: int = 18000) -> Tuple[bool, str]:
...
async def extract_audio(input_path: str, output_path: str, format: str = "mp3", bitrate: str = "192k", timeout_seconds: int = 18000) -> Tuple[bool, str]:
...
async def create_thumbnail_grid(input_path: str, output_path: str, rows: int = 3, cols: int = 3) -> Tuple[bool, str]:
    ...
    try:
        cmd = [
            'ffmpeg', '-y', '-i', input_path,
            '-vf', f'select=not(mod(n\\,{(rows*cols)+1})),scale=160:-1,tile={cols}x{rows}',
            '-frames:v', '1',
            output_path
        ]
...
async def convert_audio_format(input_path: str, output_path: str, target_format: str = "mp3", bitrate: str = "192k") -> Tuple[bool, str]:
    ...
    try:
        ...
        cmd = [
            'ffmpeg', '-y', '-i', input_path,
            '-c:a', codec,
            '-b:a', bitrate if codec != 'pcm_s16le' else '1411k',
            output_path
        ]
...
async def adjust_bitrate(input_path: str, output_path: str, video_bitrate: str = "5000k", audio_bitrate: str = "128k") -> Tuple[bool, str]:
...
async def normalize_audio(input_path: str, output_path: str) -> Tuple[bool, str]:
    ...
    try:
        cmd = [
            'ffmpeg', '-y', '-i', input_path,
            '-filter:a', 'loudnorm=I=-20:TP=-1.5:LRA=11',
            '-c:v', 'copy',
            output_path
        ]
