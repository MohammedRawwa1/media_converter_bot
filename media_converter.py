...
class ExtendedMediaConverter:
    ...
    async def extract_audio_from_video(self, input_path: str, output_path: str, format: str = 'mp3', bitrate: str = '192k') -> bool:
    ...
    async def split_video(self, input_path: str, output_pattern: str, segment_time: str = "01:00:00") -> List[str]:
        ...
        if success:
            ...
            pattern = os.path.basename(output_pattern).replace('%03d', '*')
            return sorted([f for f in os.listdir(base_dir) if f.startswith('output')])
        return []
    ...
    async def extract_subtitles(self, input_path: str, output_path: str) -> bool:
        ...
        cmd = [
            '-map', '0:s:0',
            '-c:s', 'mov_text'  # or 'copy' for original format
        ]
        return (await self.execute_ffmpeg(cmd, input_path, output_path))[0]
    ...
    async def convert_audio_format(self, input_path: str, output_path: str, target_format: str = 'mp3', quality: int = 2) -> bool:
    ...
    async def screen_record(self, output_path: str, duration: int = 10, resolution: str = "1280x720", fps: int = 30) -> bool:
